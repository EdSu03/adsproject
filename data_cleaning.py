...
def clean_parquet_files_yellow_taxi(input_dir="data", output_dir="cleaned_data"):
    ...
    # -------------------------
    file_list = glob.glob(os.path.join(input_dir, "yellow_tripdata_*.parquet"))
    ...
    print(
        f"🔍 Found {len(file_list)} files in '{input_dir}/'. Starting data cleaning...\n"
    )
    ...
    with tqdm(total=len(file_list), desc="Cleaning Progress", unit="file") as pbar:
        for file_path in file_list:
            ...
            # Explicitly set datetime columns to datetime64[us]
            df["pickup_datetime"] = df["pickup_datetime"].astype("datetime64[us]")
            df["dropoff_datetime"] = df["dropoff_datetime"].astype("datetime64[us]")

            df.drop(
                columns=["tpep_pickup_datetime", "tpep_dropoff_datetime"], inplace=True
            )
            ...
            if match:
                ...
                print(
                    f"Warning: Filename {filename} doesn't match expected format. Date filtering skipped."
                )
            ...
            ]  # Remove trips with distance < 0.05 miles
            ...
            ]  # Keep trip duration (1-180 mins)
            df = df[
                df["payment_type"].isin([1, 2, 3, 4, 5, 6])
            ]  # The possible payment types
            ...
            # -------------------------
            # 3.6.5 Ensure monetary values apart from fare_amount are non-negative
            # -------------------------
            monetary_columns = [
                "extra",
                "mta_tax",
...
def clean_parquet_files_green_taxi(input_dir="data", output_dir="cleaned_data"):
    ...
    if not file_list:
        print(
            "❌ No Green Taxi files found in './data'. Please check the folder and file names!"
        )
        return

    print(
        f"🔍 Found {len(file_list)} Green Taxi files in '{input_dir}/'. Starting data cleaning...\n"
    )
    ...
    # -------------------------
    with tqdm(
        total=len(file_list), desc="Cleaning Green Taxi Data", unit="file"
    ) as pbar:
        for file_path in file_list:
            # -------------------------
            # 3.1 Read Parquet file and convert all column names to lowercase
            ...
            df.drop(
                columns=[col for col in drop_columns if col in df.columns], inplace=True
            )
            ...
            df.drop(
                columns=["lpep_pickup_datetime", "lpep_dropoff_datetime"], inplace=True
            )
            ...
            if match:
                ...
                print(
                    f"⚠️ Warning: Filename {filename} doesn't match expected format. Date filtering skipped."
                )
            ...
            # -------------------------
            df = df[
                (df["trip_distance"] >= 0.05) & (df["trip_distance"] <= 50)
            ]  # Keep trips within 0.05 - 50 miles
